...
function research(p){const fields=p.profile.fields;const keys=p.category==='evaluation'?['executor','seed','verdict','protocol']:['object','executor','modifier','seed','verdict','protocol'];
 const selected=fields.filter(f=>keys.includes(f.key)&&f.value);
 return `<dl class="research-fields">${selected.map(f=>`<div><dt>${escapeHTML(f.label)}</dt><dd>${markdown(f.value)}</dd></div>`).join('')}</dl><details class="mechanism"><summary>查看全部研究维度与原文核对范围</summary>${profileTable(p)}${sourceCheck(p)}</details>${p.profile.missing.length?`<p class="reading-scope">待补查：${p.profile.missing.map(escapeHTML).join('、')}。可按左侧维度筛选。</p>`:''}`;
}
