...
function systemDataSource(p){const a=p.systemDataAudit;return a?`<p class="reading-scope">依据：<a href="${escapeHTML(safeURL(a.source))}" target="_blank" rel="noopener noreferrer">${escapeHTML(a.sections)} ↗</a> · ${escapeHTML(a.version)}。数据与基础系统已逐篇整理；待核实细节在对应说明中标出。</p>`:'';}
function experimentData(p){
 const value=p.profile.fields.find(f=>f.key==='protocol')?.value;
 return `<section class="experiment-data"><h3>进化、验证与测试数据</h3>${value?markdown(value):'<p class="unknown-value">尚未核实具体数据集、训练/验证/测试划分及样本量。</p>'}${systemDataSource(p)}</section>`;
}
function research(p){
 ...
 return `<dl class="research-fields">${selected.map(f=>`<div><dt>${labels[f.key]}</dt><dd>${f.value?markdown(f.value):`<span class="unknown-value">${missing[f.key]}</span>`}</dd></div>`).join('')}</dl>${experimentData(p)}<details class="mechanism"><summary>查看更新规则与全部研究维度</summary>${profileTable(p)}${sourceCheck(p)}</details>`;
}
