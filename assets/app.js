...
function groupValues(p,id){if(id==='content')return [p.readingNote?'完整专题解读':p.details.length?'原记录详细笔记':'结构化介绍'];if(id==='priority')return [p.priority];if(id==='verified')return [p.reviewed?'原文关键段落已复核':'沿用原记录'];if(id==='protocol')return [p.protocol];if(id==='depth')return p.depth;if(id==='reading')return [p.review];return p.tags;}
...
function card(p){const b=p.brief,n=p.readingNote;const intro=n?n.intro:b.summary;const conclusion=n?n.index.tldr:b.takeaway;
 return `<article class="paper-card"><div class="card-main"><div class="card-meta"><span>${CATEGORY[p.category]}${p.methodType?' / '+METHODS[p.methodType]:''}</span><span class="level level-${p.priority}">${LEVEL[p.priority]}</span>${n?'<span class="reading-badge">完整专题解读 · 6 节</span>':''}<time class="date">${escapeHTML(p.date)}</time></div><h2><button class="paper-title" data-paper="${p.id}">${escapeHTML(p.title)}</button></h2><div class="intro-copy">${markdown(intro)}</div>${conclusion?`<div class="card-takeaway"><span>这篇值得关注什么</span>${markdown(conclusion)}</div>`:b.novelty&&b.novelty!==b.summary?`<div class="novelty">${markdown(b.novelty)}</div>`:''}${n?`<div class="reading-preview"><div><b>执行者</b>${markdown(n.index.executor)}</div><div><b>修改者</b>${markdown(n.index.modifier)}</div><div><b>反馈从哪来</b>${markdown(n.index.feedback)}</div><div><b>进化与评估数据</b><p><strong>进化：</strong>${markdownInline(n.index.evolve)}</p><p><strong>评估：</strong>${markdownInline(n.index.eval)}</p></div></div>${notesCaveats(n)}`:research(p)}</div><div class="card-foot"><div class="tags">${p.tags.filter(t=>!/^M[0-3]$/.test(t)).map(t=>`<button class="tag" data-tag="${escapeHTML(t)}">#${escapeHTML(t)}</button>`).join('')}</div><div class="card-links"><a href="${escapeHTML(safeURL(p.url))}" target="_blank" rel="noopener noreferrer">论文 ↗</a><button class="read-action" data-paper="${p.id}">${n?'阅读全文解读':p.details.length?'阅读详细笔记':'阅读研究记录'} →</button></div></div></article>`;
}
