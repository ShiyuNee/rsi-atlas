...
function research(p){
 ...
 return `<dl class="research-fields">${selected.map(f=>`<div><dt>${labels[f.key]}</dt><dd>${markdown(p.overview.fields[f.key]||f.value)}<small class="field-source">${fieldSources(p,f)}</small></dd></div>`).join('')}</dl><details class="mechanism"><summary>查看更新规则与全部研究维度</summary>${profileTable(p)}</details>`;
}
function tldr(p){
 const labels={gap:'研究缺口（作者判断）',position:'本文定位',conclusion:'贡献 / 结论'};
 return `<section class="paper-tldr" aria-label="论文 TL;DR"><h3>TL;DR</h3><ol>${p.overview.tldr.map(f=>`<li><strong>${labels[f.key]}</strong><div>${markdown(f.value)}<small class="field-source">${fieldSources(p,f)}</small></div></li>`).join('')}</ol></section>`;
}
function card(p){
 return `<article class="paper-card"><div class="card-main"><div class="card-meta"><span>${CATEGORY[p.category]}${p.methodType?' / '+METHODS[p.methodType]:''}</span><span class="level level-${p.priority}">${LEVEL[p.priority]}</span><time class="date">${escapeHTML(p.date)}</time></div><h2><button class="paper-title" data-paper="${p.id}">${escapeHTML(p.title)}</button></h2>${tldr(p)}${research(p)}</div><div class="card-foot"><div class="tags">${p.tags.filter(t=>!/^M[0-3]$/.test(t)).map(t=>`<button class="tag" data-tag="${escapeHTML(t)}">#${escapeHTML(t)}</button>`).join('')}</div><div class="card-links"><a href="${escapeHTML(safeURL(p.url))}" target="_blank" rel="noopener noreferrer">论文 ↗</a><button class="read-action" data-paper="${p.id}">查看研究表 →</button></div></div></article>`;
}
