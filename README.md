...
## Benchmark-specific feedback mechanisms (2026-09-11)
...
`data/library-additions.json` 是新增资料的维护入口，每条独立记录内容类型、研究分类、日期含义、三点概览、维度说明及逐字段原文来源；`scripts/library_additions.py` 将其编译进统一索引。此前的论文记录继续使用原有维度表，互不覆盖。

- 内容类型：论文、技术报告、综述、仓库、博客，与 Methods / Evaluation 等研究分类独立筛选。
- 实证文章保留五个主要维度，完整数据／反馈／隔离说明进入同一个折叠表格。
- 综述、仓库和博客使用适用范围、分析框架、机制及证据字段；不虚构训练测试实验。
- 日期只记录来源可确认的精度。未找到明确首发日期时显示“收录日期”，不当成论文首发时间。
